...
for filename in imageslist: 
    image = cv2.imread(path+filename)
    img_gray = cv2. cvtColor(image, cv2.COLOR_BGR2GRAY)
    img_normalizedg = cv2.normalize(img_gray, None, 0, 1, cv2.NORM_MINMAX, dtype=cv2.CV_32F)
   # print(img_gray)
  ...
  #  ax1.imshow(img_normalized)
   # ax2=plt.hist(img_normalized.ravel(),256,[0,1])
    ...
    plt.hist(img_normalizedg.ravel(),256,[0,1])
...
# end of combined image historgram gray
  ...
  #  plt.subplots_adjust(left=0.1, bottom=0.09,right=0.9,top=0.9,wspace=0.4,hspace=0.4)
    ...
    plt.hist(img_normalized_b.ravel(),256,[0,1]) 
   # hist = cv2.calcHist([img_normalized_b],[0], None, [256], [0, 1])
  ...
  #  plt.subplot(3, 1, 2)
    ...
    plt.hist(img_normalized_r.ravel(),256,[0,1]) 
    plt.savefig(path+filename+"Histo-G.png") 
    plt.clf()

   # plt.subplot(3, 1, 3)
    plt.title("histogram of Green")
    plt.hist(img_normalized_g.ravel(),256,[0,1]) 
